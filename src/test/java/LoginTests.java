...
public class LoginTests extends BaseTest {
    ...
    /*Always call the setExcelSheet function as @BeforeClass annotation to make sure the sheet changes*/
    @BeforeClass
    ...
    public void testEmailFieldAcceptsVariousCharacters() {
        ...
        softAssert.assertEquals(emailInput.getAttribute("value"), testInput, "Email field does not accept all characters");
        softAssert.assertAll();
    }
    ...
    public void testPasswordFieldAcceptsVariousCharacters() {
        ...
        softAssert.assertEquals(passwordInput.getAttribute("value"), testInput, "Password field does not accept all characters");
        softAssert.assertAll();
    }
    ...
    public void testInvalidEmailValidPassword() {
        ...
        WebElement popUp = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//simple-snack-bar[@class='mat-simple-snackbar ng-star-inserted']//span[contains(text(), 'User Not Found')]")));
    ...
    public void testValidEmailInvalidPassword() {
        ...
        WebElement errorMessage = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//simple-snack-bar[@class='mat-simple-snackbar ng-star-inserted']/span[contains(text(), 'login Failed due to Incorrect email or password')]")
        ));

        softAssert.assertTrue(errorMessage.isDisplayed(), "Error message for invalid credentials is not displayed");
        softAssert.assertEquals(errorMessage.getText(), "login Failed due to Incorrect email or password", "Error message text does not match 'Invalid credentials'");
        softAssert.assertNotEquals(driver.getCurrentUrl(), "http://20.219.88.214:6102/dashboard", "Login succeeded with invalid password");
        softAssert.assertAll();
    }
    ...
    public void testShortPassword() {
        ...
        softAssert.assertEquals(passwordErrorMessage.getText(), "Please Enter Minimum 6 Characters.", "Short password error message text is incorrect");
        softAssert.assertAll();
    }
